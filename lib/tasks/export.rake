namespace :export do
  ...
  def print_current_clp_images(community_id)
    return unless CLP::LandingPageStore.enabled?(community_id)
    released_version = CLP::LandingPageStore.released_version(community_id)
    structure = CLP::LandingPageStore.load_structure(community_id, released_version)
